...
from clovers import Result, TempHandle
from clovers_apscheduler import scheduler
from .utils import LinecardDraw, download_url, download_urls
...
waifu_last_sent_time_filter = __config.waifu_last_sent_time_filter
happy_end_tips = __config.happy_end_tips
bad_end_tips = __config.bad_end_tips

waifu_he = __config.waifu_he
waifu_be = waifu_he + __config.waifu_be
waifu_ntr = __config.waifu_ntr
waifu_ntr_be = waifu_ntr + __config.waifu_be

yinpa_he = __config.yinpa_he
yinpa_cp = __config.yinpa_cp


linecard_draw = LinecardDraw(__config.fontname, __config.fallback_fonts, __config.bg_image)
"""全局渲染实例"""
waifu_data = DataBase.load(Path(__config.waifu_path) / "waifu_data.json")
"""数据库"""

groups = waifu_data.groups
protect_uids = waifu_data.protect_uids

if __config.waifu_reset:
    scheduler.add_job(waifu_data.daily_refresh_reset, "cron", hour=0, misfire_grace_time=120)
else:
    scheduler.add_job(waifu_data.daily_refresh, "cron", hour=0, misfire_grace_time=120)

if __config.at_listen:

    @PLUGIN.handle(None, ["group_id", "user_id", "at"], rule=lambda e: e.group_id is not None and bool(e.at))
    async def _(event: Event):
        assert event.group_id
        member = waifu_data.group(event.group_id).member(event.user_id)
        for uid in event.at:
            member.at_count[uid] += 1
            member.today_at_count[uid] += 1


@PLUGIN.handle(["重置娶群友记录"], ["permission"], rule=lambda e: e.permission == 3)
async def _(event: Event):
    waifu_data.daily_refresh_reset()
    return "娶群友记录已重置"


@PLUGIN.handle(["设置娶群友保护"], ["permission", "user_id", "at"], rule=lambda e: e.group_id is not None)
async def _(event: Event):
    if not event.at:
...
@PLUGIN.handle(["解除娶群友保护"], ["permission", "nickname", "user_id", "at"], rule=lambda e: e.group_id is not None)
async def _(event: Event):
    if not event.at:
...
@PLUGIN.handle(["查看娶群友保护名单"], ["group_id"], rule=lambda e: e.group_id is not None)
async def _(event: Event):
    assert event.group_id
...
@PLUGIN.handle(["娶群友"], ["group_id", "user_id", "at", "avatar"], rule=lambda e: e.group_id is not None)
async def _(event: Event):
    assert event.group_id
    ...
    if event.at and (waifu_id := event.at[0]) != user_id:  # 如果 at 了别的群友
        if couple_id:  # 在自己有 CP 的情况下
            if couple_id == waifu_id:  # 如果 at 到自己的 CP 直接进入 HE
                ...
                if coro := event.call("group_member_info", group_id, couple_id):
                    waifu.update(await coro)
                member = group.member(user_id)
                waifu_data.save()
                message = [f"你们已经是CP了哦~\n你的CP是【{waifu.name}】"]
                message.extend(statistic(member, waifu))
                message.append(random.choice(happy_end_tips))
                return [Result("at", user_id), "\n".join(message), Result("image", waifu.avatar)]
                ...
                if randvalue <= waifu_he:  # 成功指定
                    ...
                    if coro := event.call("group_member_info", group_id, waifu_id):
                        waifu.update(await coro)
                    waifu_data.save()
                    return [Result("at", user_id), f"恭喜你娶到了群友！【{waifu.name}】", Result("image", waifu.avatar)]
                elif randvalue <= waifu_be:  # 锁定单身
                    group.couple[user_id] = user_id
                    group.member(user_id).cp_count[user_id] += 1
                    waifu_data.save()
                    return [Result("at", user_id), f"{random.choice(bad_end_tips)}\n恭喜你娶到了你自己。", Result("image", event.avatar)]
                else:  # 失败指定
                    return [Result("at", user_id), f"{random.choice(bad_end_tips)}\n你没有娶到群友。"]
            else:  # 如果自己主动锁定了自己现在的 CP，则不允许再配对
                waifu = group.member(couple_id)
                return [Result("at", user_id), f"你已经有CP了，不许花心哦~\n你的CP：【{waifu.name}】\n", Result("image", waifu.avatar)]
            ...
            if group.in_locking(waifu_id) or ((randvalue := random.randint(1, 100)) > waifu_ntr_be):
                return [Result("at", user_id), tips, Result("image", waifus_waifu.avatar)]
            if randvalue <= waifu_ntr:  # 触发 NTR HE 判定
                ...
                waifu_data.save()
                tips += f"\n但是...\n恭喜你抢到了群友{waifu.name}"
                return [Result("at", user_id), tips, Result("image", waifu.avatar)]
            else:  # 触发 NTR BE 判定
                group.couple[user_id] = user_id
                group.member(user_id).cp_count[user_id] += 1
                waifu_data.save()
                tips += "不过好消息是...\n你娶到了你自己！"
                return [Result("at", user_id), tips, Result("image", event.avatar)]
        ...
        if couple_id:  # 自己有 CP 记录，直接配对到 CP，否则随机配对
            ...
            if coro := event.call("group_member_list", group_id):
                group.update(await coro)
...
async def confirm(event: Event, handle: TempHandle):
    match event.message:
        case "确认":
            ...
            waifu_data.save()
            handle.finish()
            return [Result("at", event.user_id), "你们已经是单身了。"]
        case "取消":
            handle.finish()
            return [Result("at", event.user_id), "你们要幸福哦~"]


@PLUGIN.handle(["离婚", "分手"], ["group_id", "user_id", "to_me"], rule=lambda e: e.to_me and e.group_id is not None)
async def _(event: Event):
    assert event.group_id
    group_id = event.group_id
    user_id = event.user_id
    ...
    if group.in_locking(user_id):
        PLUGIN.temp_handle(["group_id", "user_id"], rule=lambda e: e.user_id == user_id and e.group_id == group_id)(confirm)
        return [Result("at", user_id), "你们已经互相锁定了，请问真的要分手吗？【确认 or 取消】"]
    group.disband(user_id)
    waifu_data.save()
    return [Result("at", user_id), "你们已经是单身了。"]


@PLUGIN.handle(["查看娶群友卡池"], ["group_id"], rule=lambda e: e.group_id is not None)
async def _(event: Event):
    assert event.group_id
    group = waifu_data.group(event.group_id)
    ...
    return linecard_draw.waifu_list("群友卡池[font size = 30,color = gray]（前20位）", list(zip(avatars, texts)))


@PLUGIN.handle(["本群CP", "本群cp"], ["group_id"], rule=lambda e: e.group_id is not None)
async def _(event: Event):
    assert event.group_id
    group = waifu_data.group(event.group_id)
    if not group.couple:
    ...
    return linecard_draw.couple("本群CP", list(zip(p1_avatars, p1_names, p0_avatars, p0_names)))


@PLUGIN.handle(["透群友"], ["group_id", "user_id", "at"], rule=lambda e: e.group_id is not None)
async def _(event: Event):
    assert event.group_id
    group_id = event.group_id
    user_id = event.user_id
    ...
    if event.at:
        ...
        if coro := event.call("group_member_info", group_id, pet_id):
            pet.update(await coro)
    else:
        if coro := event.call("group_member_list", group_id):
            group.update(await coro)
    ...
    if pet_id == waifu_id:
        ...
        tips = "恭喜你涩到了群友！"
    return [Result("at", user_id), f"{tips}\n你的涩涩对象是：【{pet.name}】", Result("image", pet.avatar)]


@PLUGIN.handle(["色色记录", "涩涩记录"], ["group_id"], rule=lambda e: e.group_id is not None)
async def _(event: Event):
    assert event.group_id
    group = waifu_data.group(event.group_id)
    output = []
    ...
    if p1_data:
        output.append(linecard_draw.sese("透群友记录", p1_data))
    if p0_data:
        output.append(linecard_draw.sese("群友被透记录", p0_data))
