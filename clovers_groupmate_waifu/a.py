...
async def waifu_rule(bot: Bot, event: GroupMessageEvent, state: T_State) -> bool:
    ...
    if not waifu_id:
        ...
        waifu_ids = [
            user_id for member in member_list if (user_id := member["user_id"]) not in rule_out and member["last_sent_time"] > lastmonth
        ]
        if waifu_ids:
...
if waifu_cd_bye > -1:
    ...
    bye = on_command(
        ...
        and record_CP[event.group_id].get(event.user_id, event.user_id) != event.user_id,
        priority=90,
        block=True,
    )
